...
class _ChatPageState extends State<ChatPage> with TickerProviderStateMixin {
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      if (cards.isEmpty) {
        cards = [_SuggestionCard(type: 'insight', title: 'Seasonal Guidance', subtitle: 'A gentle moment with nature')];
      }
      ...
      setState(() {
        _messages.add(_ChatMessage(
          ...
          text: offlineText,
          cards: [
            _SuggestionCard(type: 'diet', title: 'Dietary Care', subtitle: 'Nourish with tremella and lily bulbs', icon: Icons.restaurant),
            _SuggestionCard(type: 'rest', title: 'Rest Suggestions', subtitle: 'Sleep early, wake early, follow nature\'s rhythm', icon: Icons.bedtime),
          ],
  ...
  void _showUpgradePrompt() {
    showModalBottomSheet(context: context, backgroundColor: Colors.transparent, builder: (_) => Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const Text('Daily messages used up', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: ShunShiColors.textPrimary)),
        ...
        const SizedBox(height: 20),
        SizedBox(width: double.infinity, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: ShunShiColors.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: ShunShiRadius.buttonRadius)), onPressed: () { Navigator.pop(context); context.push('/subscription'); }, child: const Text('Upgrade Now', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)))),
        const SizedBox(height: 10),
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('Later', style: TextStyle(color: ShunShiColors.textTertiary))),
        const SizedBox(height: 8),
      ]),
    ));
  }
  ...
  void _showSafetyDialog() {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: Row(children: [
        ...
        const Text('Health Safety Notice', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w500, color: ShunShiColors.textPrimary)),
      ]),
      content: const Text(
      ...
      actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('I Understand', style: TextStyle(color: ShunShiColors.primary, fontWeight: FontWeight.w600)))],
    ));
  }

  String _offlineReply(String msg) {
    if (msg.contains('睡') || msg.contains('insomnia') || msg.contains('sleep')) return 'Sleep issues often relate to an unsettled mind. During $_currentSolarTerm, try soaking your feet in warm water for 15 minutes before bed, put down your phone, and let your mind quiet down.';
    if (msg.contains('累') || msg.contains('tired') || msg.contains('fatigue')) return 'Feeling tired during seasonal transitions is normal. Consider reducing workload and eating warming, nourishing foods. A cup of ginger-date tea in the morning can help restore vitality.';
    ...
    return Padding(padding: const EdgeInsets.only(bottom: 20), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          const SizedBox(width: 8),
          GestureDetector(onTap: () { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Playing audio...'), duration: Duration(seconds: 2))); }, child: const Icon(Icons.volume_up, size: 14, color: ShunShiColors.textTertiary)),
        ]),
      ])),
    ]));
  }
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Container(constraints: const BoxConstraints(maxHeight: 120), child: TextField(controller: _controller, focusNode: _focusNode, maxLines: null, textInputAction: TextInputAction.send, onSubmitted: (_) => _sendMessage(),
          decoration: InputDecoration(hintText: 'Ask about wellness...', hintStyle: const TextStyle(fontSize: 14, color: ShunShiColors.textTertiary), filled: true, fillColor: ShunShiColors.surface, contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10), border: OutlineInputBorder(borderRadius: BorderRadius.circular(20), borderSide: BorderSide.none)))),
        ),
        const SizedBox(width: 8),
    ...
    showDialog(context: context, barrierDismissible: true, builder: (dc) {
      return StatefulBuilder(builder: (dc, setDialogState) {
        ...
        return PopScope(canPop: true, child: AlertDialog(
          ...
          actions: [TextButton(onPressed: () { closed = true; Navigator.of(dc).pop(); }, child: const Text('Cancel'))],
        ));
      });
    });
  }
  ...
  void _showQuickTopics() {
    showModalBottomSheet(context: context, backgroundColor: ShunShiColors.background, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(10))),

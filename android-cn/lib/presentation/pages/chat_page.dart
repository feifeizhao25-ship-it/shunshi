...
class _ChatPageState extends State<ChatPage> with TickerProviderStateMixin {
  ...
  bool _containsBlocked(String text) =>
      _blockedKeywords.any((k) => text.contains(k));
        ...
        _messages.add(
          ChatMessage(
            ...
            cards: [
              SuggestionCard(
                type: 'greeting',
                title: '当前：$_currentSolarTerm',
                subtitle: _solarTermDesc,
              ),
            ],
  ...
  Future<void> _loadConversation(SharedPreferences prefs) async {
    ...
    _conversationId =
        prefs.getString('conversation_id') ?? widget.conversationId;
    if (saved != null && saved.isNotEmpty) {
      final lines = saved.split('\n');
      final recent = lines.length > 10
  ...
  Future<void> _sendMessage() async {
    ...
    if (AISafetyService.isPromptInjection(text)) {
      setState(() {
        _messages.add(
          ChatMessage(role: 'user', text: text, time: _fmt(DateTime.now())),
        );
        _messages.add(
          ChatMessage(
            role: 'ai',
            text: '检测到异常输入，请重新描述您的问题。',
            time: _fmt(DateTime.now()),
          ),
        );
      });
            ...
            cards: riskLevel == RiskLevel.crisis
                ? [
                    SuggestionCard(
                      type: 'crisis',
                      title: '紧急求助热线',
                      subtitle: '400-161-9995 · 24小时心理援助',
    ...
    try {
      ...
      try {
        ...
        final v2Res = await v2Dio.post(
          '/api/v1/chat?message=${Uri.encodeComponent(text)}&conversation_id=${_conversationId ?? ''}',
        );
        final v2Data = v2Res.data;
        final aiText =
            v2Data?['data']?['text'] ?? v2Data?['reply'] ?? '抱歉，暂时无法回应。';
        final convId =
            v2Data?['data']?['conversation_id'] ?? v2Data?['conversation_id'];
        ...
        if (so != null) {
          ...
          if (actions != null) {
            for (var a in actions) {
              final duration = a['duration_min'] != null
                  ? '约${a['duration_min']}分钟'
                  : '';
              v2Cards.add(
                SuggestionCard(
                  type: a['type'] ?? 'tip',
                  title: a['title'] ?? '',
                  subtitle: '${a['description'] ?? ''} $duration'.trim(),
        ...
        final v2Sources = (so?['disclaimer'] != null)
            ? ['⚠️ ${so?["disclaimer"]}']
            : <String>[];
        handleAIReply(
          aiText,
        ...
        handleAIReply(
          ...
          cards: [
            SuggestionCard(
              type: 'info',
              title: '服务暂不可用',
              subtitle: '请检查网络后重试，离线状态不生成健康建议',
              icon: Icons.cloud_off,
            ),
          ],
          sources: const ['离线模式｜未调用知识库｜没有生成健康结论'],
        );
      }
    }
    _scrollToBottom();
  ...
  String _offlineReply() {
    return '当前无法连接顺时服务，因此没有调用已核验知识库，也不会在离线状态生成健康建议。请检查网络后重试；如有明显或持续不适，请及时咨询医疗专业人员。';
  }
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: ShunShiColors.gold.withValues(alpha: 0.1),
            ...
            TextButton(
              ...
              child: const Text(
                '稍后再说',
                style: TextStyle(color: ShunShiColors.textTertiary),
              ),
            ),
            const SizedBox(height: 8),
          ],
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Text(
                '内容由AI生成，仅供健康养生参考，不构成医疗诊断建议。如有不适请及时就医。',
          ...
          Row(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: _isVip
                      ? ShunShiColors.gold.withValues(alpha: 0.15)
                  ...
                  children: [
                    if (_isVip) ...[
                      ...
                      Icon(
                        ...
                        size: 13,
                        color: _quotaRemaining > 3
                            ? ShunShiColors.primary
                            : Colors.orange,
                      ),
                      const SizedBox(width: 3),
                      Text(
                        '$_quotaRemaining/$_quotaLimit',
                        style: TextStyle(
                          fontSize: 11,
  ...
  Widget _buildList() {
    ...
    if (_messages.isEmpty && _isLoading) {
      return ListView(
        children: [
          const SkeletonMessage(isAI: true),
    ...
    return ListView.builder(
      ...
      itemCount:
          _messages.length + (_isLoading ? 1 : 0) + (showUpgradeNudge ? 1 : 0),
      itemBuilder: (context, i) {
        if (i == 0 && showUpgradeNudge) {
          return Column(
            children: [
              UpgradeNudge(onTap: _showUpgradePrompt),
              const SizedBox(height: 16),
            ],
          );
        }
                    ...
                    builder: (context, child) {
                      ...
                      return Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: focused
                              ? Border.all(
                                  color: ShunShiColors.primary.withValues(
                                    alpha: 0.6,
                                  ),
                                  width: 1.5,
                                )

...
API_BASE_CN="${API_BASE_CN:-http://localhost:4000/api/v1}"
...
build_android() {
    ...
    (
        ...
        if [[ -f "$apk_src" ]]; then
            # Sign APK manually if not already signed by Gradle
            if command -v jarsigner &>/dev/null; then
                local keystore="android/keystore/release.jks"
                local keyprop="android/key.properties"
                if [[ -f "$keystore" && -f "$keyprop" ]]; then
                    local storepass=$(grep '^storePassword=' "$keyprop" | cut -d= -f2)
                    local keypass=$(grep '^keyPassword=' "$keyprop" | cut -d= -f2)
                    local keyalias=$(grep '^keyAlias=' "$keyprop" | cut -d= -f2)
                    local signed_apk="${apk_src%.apk}-signed.apk"
                    jarsigner -keystore "$keystore" -storepass "$storepass" -keypass "$keypass" \
                        -signedjar "$signed_apk" "$apk_src" "$keyalias" 2>/dev/null && \
                        apk_src="$signed_apk" && log "  APK signed"
                fi
            fi

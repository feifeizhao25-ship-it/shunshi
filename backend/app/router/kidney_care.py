...
async def get_tcm_functions():
    return {"success": True, "data": KIDNEY_TCM}
...
async def get_deficiency_type(type_id: str):
    ...
    return {"success": True, "data": KIDNEY_DEFICIENCY_TYPES[type_id]}


@router.get("/foods", summary="补肾食物推荐")
async def get_kidney_foods():
    return {"success": True, "data": {"foods": KIDNEY_FOODS, "tip": "五行中黑色食物入肾，补肾食物以黑色为主"}}


@router.get("/acupoints", summary="护肾穴位")
async def get_kidney_acupoints():
    return {"success": True, "data": {"acupoints": KIDNEY_ACUPOINTS}}
...
async def get_daily_plan():
    plan = {
        "morning": "起床后按揉涌泉穴5分钟",
        "afternoon_peak": "酉时（17-19点）肾经当令，此时按摩太溪穴效果最佳",
        "exercise": "腰部伸展运动，俯卧撑、深蹲等强肾功法",
        "diet": "每餐加入黑色食物",
        "sleep": "23点前入睡，充分休息养精蓄锐",
        "foot_bath": "睡前热水泡脚，加入生姜或艾叶，温肾助眠"
    }

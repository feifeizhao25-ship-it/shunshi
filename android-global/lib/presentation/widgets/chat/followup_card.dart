...
/// Follow-up Card - Post-conversation follow-up reminders from AI
class FollowUpCard extends StatelessWidget {
  final Map<String, dynamic> followUpData;
  final ValueChanged<String>? onDismiss;
  final ValueChanged<String>? onSnooze;

  const FollowUpCard({super.key, required this.followUpData, this.onDismiss, this.onSnooze});
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(title, style: SeasonsTextStyles.heading.copyWith(fontWeight: FontWeight.w600)),
                      if (scheduledAt.isNotEmpty)
                        Text(_formatTime(scheduledAt), style: SeasonsTextStyles.caption.copyWith(color: SeasonsColors.textHint)),
                    ],
                  ),
                ),
                if (priority == 'high')
                  Container(
                    ...
                    decoration: BoxDecoration(color: SeasonsColors.error.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(4)),
                    child: Text('Priority', style: SeasonsTextStyles.overline.copyWith(color: SeasonsColors.error)),
                  ),
              ],
            ),
          ),
              ...
              children: [
                Expanded(
                  child: OutlinedButton(
                    ...
                    style: OutlinedButton.styleFrom(
                      side: BorderSide(color: SeasonsColors.border),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                      padding: const EdgeInsets.symmetric(vertical: 8),
                    ),
                    child: Text('Dismiss', style: SeasonsTextStyles.buttonSmall.copyWith(color: SeasonsColors.textSecondary)),
                  ),
                ),
                ...
                Expanded(
                  child: FilledButton(
                    ...
                    child: Text('Snooze', style: SeasonsTextStyles.buttonSmall.copyWith(color: Colors.white)),
                  ),
                ),
              ],
            ),
  ...
  Color _getColor(String type) {
    switch (type) {
      case 'daily_checkin': return SeasonsColors.sky;
      case 'mood_followup': return SeasonsColors.success;
      case 'sleep_followup': return const Color(0xFF7E57C2);
      case 'care_reminder': return SeasonsColors.warm;
      case 'subscription_expiring': return SeasonsColors.error;

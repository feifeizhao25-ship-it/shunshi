...
class _FavoritesPageState extends State<FavoritesPage> {
  ...
  Future<void> _loadFavorites() async {
    ...
    try {
      ...
      if (mounted) setState(() { _loading = false; _error = 'Load failed'; });
    }
  }
  ...
  String _typeLabel(String? type) {
    const labels = {'diet': 'Food Therapy', 'tea': 'Herbal Tea', 'recipe': 'Recipes', 'herb': 'Formula', 'meridian': 'Meridian', 'exercise': 'Qigong', 'acupoint': 'Acupressure', 'sleep': 'Calm Mind', 'emotion': 'Emotional'};
  ...
  Future<void> _removeFavorite(int index) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.of(context).t('favorites_failed_to_remove_from_favorites'))));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: isDark ? ShunshiDarkColors.background : ShunShiColors.background,
      appBar: AppBar(
        ...
        title: Text('${AppLocalizations.of(context).get('library_favorites')} (${_items.length})',

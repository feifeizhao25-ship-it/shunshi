...
class TodayPlanService {
  ...
  Future<TodayPlan> getTodayPlan() async {
    try {
      final response = await _dio.get('/api/v1/today-plan?locale=en-US');
  ...
  Future<TodayPlan> refreshPlan() async {
    try {
      final response = await _dio.post('/api/v1/today-plan/refresh?locale=en-US');
      return TodayPlan.fromJson(response.data);
    } catch (e) {
      return _getDefaultPlan();
    }
    ...
    return TodayPlan(
      ...
      insights: [
        const TodayInsight(
          id: '1',
          title: 'Good Morning',
          content: 'A new day begins. Remember to drink warm water',
          icon: '🌅',
      ...
      tasks: [
        const FollowUpTask(
          ...
          description: 'Helps detoxify the body',
      ...
      solarTerm: SolarTermCard(
        name: 'Spring Equinox',
        emoji: '🌱',
        description: 'Yin-Yang balance, equal day and night',
        suggestions: ['Moderate exercise', 'Regulate Spleen & Stomach'],
        date: DateTime.now(),
      ),
    );
  }

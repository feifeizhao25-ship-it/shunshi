...
class PersonalizationService {
  ...
  static Future<PersonalizedData> fetchPersonalizedData(String? userId) async {
    ...
    final results = await Future.wait([
      _safeGet('/api/v1/constitution/result/${userId ?? ''}'),          // 0: Constitution
      _safeGet('/api/v1/recommendations/daily'),                         // 1: Daily recommendations
      _safeGet('/api/v1/exercise/daily'),                                // 2: Daily exercise
      _safeGet('/api/v1/tea/daily'),                                     // 3: Today's tea
      _safeGet('/api/v1/food-therapy/recommend'),                        // 4: Food therapy recommendations
      _safeGet('/api/v1/ai/daily-insight'),                              // 5: AI insights
      _safeGet('/api/v1/acupoints/daily'),                               // 6: Daily acupoints
      _safeGet('/api/v1/solar-wellness/daily-advice'),                   // 7: Seasonal wellness
    ]);
